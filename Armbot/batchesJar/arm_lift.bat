...
"%JAVA_BIN%"\java -cp . -p .;"%JAVAFX_HOME%";jars\Basics.jar;jars\Manipulators.jar;jars\Platform.jar;jars\Robotinterface.jar;jars\Simulator.jar --add-modules Manipulators,Robotcontroller -m Simulator/kin.sim.ArmBot -logdir logs -logall -renderflags FFT -man manipulators.Lift -gri manipulators.LinearGripper -c robotcontroller.LiftTest -e environments\environment_gripball.txt
